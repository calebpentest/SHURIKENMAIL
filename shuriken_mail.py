...
class Tooltip:
    ...
    def show_tooltip(self, event=None):
        ...
        label = tk.Label(self.tooltip_window, text=self.text, background="#4A4A4A", foreground="#FFFFFF",
...
class ShurikenMail:
    ...
    def setup_gui(self):
        ...
        self.toolbar = tk.Frame(self.root, bg="#4A4A4A", height=30)
        self.toolbar.pack(fill=tk.X)
        self.logo_label = tk.Label(self.toolbar, text="ShurikenMail", font=("Segoe UI Semibold", 12), fg="#FFFFFF", bg="#4A4A4A")
        self.logo_label.pack(side=tk.LEFT, padx=10)
        self.save_config_button = tk.Button(self.toolbar, text="Save", font=("Segoe UI", 9), bg="#4A4A4A", fg="#FFFFFF", bd=0, command=self.save_config)
        self.save_config_button.pack(side=tk.LEFT, padx=10)
        Tooltip(self.save_config_button, "Save configuration (Ctrl+S)")
        self.clear_form_button = tk.Button(self.toolbar, text="Clear", font=("Segoe UI", 9), bg="#4A4A4A", fg="#FFFFFF", bd=0, command=self.clear_form)
        ...
        for btn in [self.save_config_button, self.clear_form_button]:
            btn.bind("<Enter>", lambda e, b=btn: b.config(bg="#5A5A5A"))
            btn.bind("<Leave>", lambda e, b=btn: b.config(bg="#4A4A4A"))
        self.sidebar = tk.Frame(self.root, bg="#4A4A4A", width=50)
        self.sidebar.pack(side=tk.LEFT, fill=tk.Y)
        self.consent_var = tk.BooleanVar()
        self.consent_frame = tk.Frame(self.sidebar, bg="#4A4A4A")
        ...
        self.consent_label.bind("<Enter>", lambda e: self.consent_label.config(background="#5A5A5A"))
        self.consent_label.bind("<Leave>", lambda e: self.consent_label.config(background="#4A4A4A"))
        Tooltip(self.consent_label, "Confirm authorized use only\nRequired to send emails")
        self.main_frame = tk.Frame(self.root, bg="#4A4A4A")
        ...
        self.save_keyring_button = tk.Button(self.smtp_frame, text="Save password", font=("Segoe UI", 9), bg="#4A4A4A", fg="#FFFFFF", bd=0, command=self.save_password)
        self.save_keyring_button.grid(row=3, column=2, padx=10)
        self.save_keyring_button.bind("<Enter>", lambda e: self.save_keyring_button.config(bg="#5A5A5A"))
        self.save_keyring_button.bind("<Leave>", lambda e: self.save_keyring_button.config(bg="#4A4A4A"))
        ...
        self.targets_text = tk.Text(self.content_frame, height=4, width=50, font=("Segoe UI", 9), bg="#4A4A4A", fg="#FFFFFF")
        ...
        self.load_csv_button = tk.Button(self.content_frame, text="Load CSV", font=("Segoe UI", 9), bg="#4A4A4A", fg="#FFFFFF", bd=0, command=self.load_csv)
        self.load_csv_button.grid(row=1, column=2, padx=10)
        self.load_csv_button.bind("<Enter>", lambda e: self.load_csv_button.config(bg="#5A5A5A"))
        self.load_csv_button.bind("<Leave>", lambda e: self.load_csv_button.config(bg="#4A4A4A"))
        ...
        self.message_text = tk.Text(self.content_frame, height=5, width=50, font=("Segoe UI", 9), bg="#4A4A4A", fg="#FFFFFF")
        ...
        self.browse_attach_button = tk.Button(self.content_frame, text="Add Attachments", font=("Segoe UI", 9), bg="#4A4A4A", fg="#FFFFFF", bd=0, command=self.browse_attachments)
        self.browse_attach_button.grid(row=6, column=2, padx=10)
        self.browse_attach_button.bind("<Enter>", lambda e: self.browse_attach_button.config(bg="#5A5A5A"))
        self.browse_attach_button.bind("<Leave>", lambda e: self.browse_attach_button.config(bg="#4A4A4A"))
        ...
        self.preview_text = tk.Text(self.preview_frame, height=8, width=50, font=("Segoe UI", 9), bg="#4A4A4A", fg="#FFFFFF", state="disabled")
        ...
        self.update_preview_button = tk.Button(self.preview_buttons_frame, text="Refresh", font=("Segoe UI", 9), bg="#4A4A4A", fg="#FFFFFF", bd=0, command=self.update_preview)
        self.update_preview_button.pack(side=tk.LEFT, padx=10)
        self.update_preview_button.bind("<Enter>", lambda e: self.update_preview_button.config(bg="#5A5A5A"))
        self.update_preview_button.bind("<Leave>", lambda e: self.update_preview_button.config(bg="#4A4A4A"))
        Tooltip(self.update_preview_button, "Refresh preview (Ctrl+P)")
        self.test_send_button = tk.Button(self.preview_buttons_frame, text="Test Send", font=("Segoe UI", 9), bg="#4A4A4A", fg="#FFFFFF", bd=0, command=self.test_send)
        self.test_send_button.pack(side=tk.LEFT, padx=10)
        self.test_send_button.bind("<Enter>", lambda e: self.test_send_button.config(bg="#5A5A5A"))
        self.test_send_button.bind("<Leave>", lambda e: self.test_send_button.config(bg="#4A4A4A"))
        Tooltip(self.test_send_button, "Send test email to your address")
        self.action_frame = tk.Frame(self.main_frame, bg="#4A4A4A")
        self.action_frame.pack(fill=tk.X, pady=10)
        self.send_button = tk.Button(self.action_frame, text="Send Emails", font=("Segoe UI", 12), bg="#4A4A4A", fg="#FFFFFF", bd=0, command=self.start_sending, width=12)
        self.send_button.pack(side=tk.LEFT, padx=10)
        self.send_button.bind("<Enter>", lambda e: self.send_button.config(bg="#5A5A5A"))
        self.send_button.bind("<Leave>", lambda e: self.send_button.config(bg="#4A4A4A"))
        Tooltip(self.send_button, "Send emails to recipients (Ctrl+Enter)")
        self.status_frame = tk.Frame(self.main_frame, bg="#4A4A4A")
        ...
        self.log_toggle_button = tk.Button(self.status_frame, text="View Logs", font=("Segoe UI", 9), bg="#4A4A4A", fg="#FFFFFF", bd=0, command=self.toggle_log_viewer)
        self.log_toggle_button.pack(side=tk.RIGHT, padx=10)
        self.log_toggle_button.bind("<Enter>", lambda e: self.log_toggle_button.config(bg="#5A5A5A"))
        self.log_toggle_button.bind("<Leave>", lambda e: self.log_toggle_button.config(bg="#4A4A4A"))
        Tooltip(self.log_toggle_button, "Show/hide log viewer")
        self.log_frame = tk.Frame(self.main_frame, bg="#4A4A4A")
        self.log_text = tk.Text(self.log_frame, height=5, font=("Segoe UI", 9), bg="#4A4A4A", fg="#FFFFFF", state="disabled")
        self.log_text.pack(fill=tk.X, padx=10, pady=5)
        self.reset_log_button = tk.Button(self.log_frame, text="Reset Logs", font=("Segoe UI", 9), bg="#4A4A4A", fg="#FFFFFF", bd=0, command=self.reset_fernet_key)
        self.reset_log_button.pack(pady=5)
        self.reset_log_button.bind("<Enter>", lambda e: self.reset_log_button.config(bg="#5A5A5A"))
        self.reset_log_button.bind("<Leave>", lambda e: self.reset_log_button.config(bg="#4A4A4A"))
        Tooltip(self.reset_log_button, "Reset encryption key and clear logs")
        self.log_visible = False
        self.notification_frame = tk.Frame(self.main_frame, bg="#4A4A4A", relief="solid", borderwidth=1)
        self.notification_frame.pack(fill=tk.X, pady=10)
        self.notification_label = ttk.Label(self.notification_frame, text="", font=("Segoe UI", 9), background="#4A4A4A", foreground="#FFFFFF")
    ...
    def configure_styles(self):
        styles = {
            "TLabel": {"background": "#4A4A4A", "foreground": "#FFFFFF", "font": ("Segoe UI", 9), "padding": 5},
            "TEntry": {"fieldbackground": "#4A4A4A", "foreground": "#FFFFFF", "font": ("Segoe UI", 9), "borderwidth": 1, "relief": "flat"},
            "TCombobox": {"fieldbackground": "#4A4A4A", "foreground": "#FFFFFF", "font": ("Segoe UI", 9), "borderwidth": 1},
            "TNotebook": {"background": "#4A4A4A", "padding": 5},
            "TNotebook.Tab": {"background": "#4A4A4A", "foreground": "#FFFFFF", "font": ("Segoe UI Semibold", 9), "padding": [10, 5]},
            "TProgressbar": {"background": "#4A4A4A", "troughcolor": "#4A4A4A", "foreground": "#FFFFFF"}
        }
        for widget, config in styles.items():
            self.style.configure(widget, **config)
        self.style.map("TNotebook.Tab", background=[("selected", "#4A4A4A"), ("active", "#5A5A5A")], foreground=[("selected", "#FFFFFF")])
